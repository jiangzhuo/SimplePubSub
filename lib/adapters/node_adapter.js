...
var logger = new SimplePubSub.Logging('NodeAdapter');
class NodeAdapter {
    ...
    attach(server) {
        if (server instanceof http.Server || server instanceof https.Server) {
            logger.debug('attach http(s) server');
            ...
            logger.debug('attach net server');
    ...
    _callWithParams(request, response, params) {
        ...
        try {
            logger.debug('Received message via HTTP ' + request.method + ': %s', params.message);
            ...
            this._server.process(message, request, function (replies) {
                ...
                logger.debug('HTTP response: %s', body);
    ...
    handleUpgrade(request, socket, head) {
        ...
        ws.onmessage = function (event) {
            try {
                logger.debug('Received message via WebSocket[' + ws.version + ']: %s', event.data);

                var message = JSON.parse(event.data),
                    cid = SimplePubSub.clientIdFromMessages(message);

                logger.debug('cid and clientId', cid, clientId);
    ...
    handleConnection(socket) {
        ...
        socket.on('data', function (data) {
            ...
            try {
                logger.debug('Received message via Socket', data.toString());
    ...
    handleEventSource(request, response) {
        ...
        logger.debug('Opened EventSource connection for %s', clientId);
