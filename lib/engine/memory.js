...
var logger = new (require('../logging.js'))('Memory');
class Memory extends Timeouts {
    ...
    createClient(callback, context) {
        ...
        logger.debug('Created new client %s', clientId);
        this.ping(clientId);
    ...
    destroyClient(clientId, callback, context) {
        ...
        logger.debug('Destroyed client %s', clientId);
    ...
    ping(clientId) {
        ...
        logger.debug('Ping %s, %s', clientId, timeout);
        this.removeTimeout(clientId);
    ...
    subscribe(clientId, channel, callback, context) {
        ...
        logger.debug('Subscribed client %s to channel %s', clientId, channel);
    ...
    unsubscribe(clientId, channel, callback, context) {
        ...
        logger.debug('Unsubscribed client %s from channel %s', clientId, channel);
        if (trigger) this._server.emit('unsubscribe', clientId, channel);
        if (callback) callback.call(context, true);
    }

    publish(message, channels) {
        logger.debug('Publishing message %s', JSON.stringify(message));
        ...
        for (let clientId of clients) {
            logger.debug('Queueing for client %s: %s', clientId, message);
