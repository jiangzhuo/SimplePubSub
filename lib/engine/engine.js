...
class Engine extends EventEmitter {
    constructor(options) {
        ...
        console.log('Created new engine: %s', JSON.stringify(this._options));
    }

    connect(clientId, options, callback, context) {
        console.log('Accepting connection from %s', clientId);
    ...
    closeConnection(clientId) {
        console.log('Closing connection for %s', clientId);
        var conn = this._connections[clientId];
        if (!conn) return;
    ...
    flushConnection(clientId, close) {
        if (!clientId) return;
        console.log('Flushing connection for %s', clientId);
        var conn = this.connection(clientId, false);
        if (!conn) return;
